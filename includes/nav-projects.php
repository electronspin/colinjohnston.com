<!-- Recent projects -->

<li class="nav-list-title">Featured Projects</li>

<li <?php echo ($page=='rollbar-account-dashboard' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/rollbar-account-dashboard">
        <?php echo $rbad ?>
    </a>
</li>

<li <?php echo ($page=='solanolabs-ci-session-view' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/solanolabs-ci-session-view">
        <?php echo $slcs ?>
    </a>
</li>

<li <?php echo ($page=='paul-saffo-website' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/paul-saffo-website">
        <?php echo $psws ?>
    </a>
</li>

<!-- More projects -->

<li class="show-for-small nav-list-title">&nbsp;</li>

<li class="nav-list-title">More Projects</li>

<li <?php echo ($page=='rollbar-marketing-website' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/rollbar-marketing-website">
        <?php echo $rbws ?>
    </a>
</li>
<li <?php echo ($page=='rollbar-identity-refresh' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/rollbar-identity-refresh">
        <?php echo $rbid ?>
    </a>
</li>

<li <?php echo ($page=='moovweb-dev-center' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/moovweb-dev-center">
        <?php echo $mwdc ?>
    </a>
</li>
<li <?php echo ($page=='moovweb-control-center' ) ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
    <a href="/projects/moovweb-control-center">
        <?php echo $mwcc ?>
    </a>
</li>
<!-- <li <?php echo ($page == 'the-real-sf-tour-website') ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
            <a href="/projects/the-real-sf-tour-website">The Real SF Tour Website</a>
        </li>
        <li <?php echo ($page == 'orphic-gallery-website') ? "class='nav-list-item active'" : "class='nav-list-item'"; ?>>
            <a href="/projects/orphic-gallery-website">Orphic Gallery Website</a>
        </li> -->
<li class="nav-list-title">&nbsp;</li>
<li class="show-for-small">
    <a href="/connect-and-collaborate">Contact</a>
</li>
<li class="show-for-small">
    <a href="/">Home</a>
</li>
