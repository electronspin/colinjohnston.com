...
function get_post_names() {
  ...
  if(empty($_cache)){
    ...
    // Get the names of al lthe posts (newest first):
...
// Retun an array of posts
// can return a subset of the posts
...
function get_posts($page = 1, $perpage = 0){
  ...
  // Create new instance of the Markdown parser
...
function generate_rss($posts){
  ...
  $channel
    ...
    ->description(config('blog.decription'))
    ->url($site_url())
    ->appendTo($feed);
